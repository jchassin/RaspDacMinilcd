...
function parseStatus($resp) {
        workerLog('parseStatus(): parseStatus');
...
function parseCurrentSong($sock) {
	...
	$resp = readMpdResp($sock);
	workerLog('parseCurrentSong : ' . $resp);
	...
	if (is_null($resp) ) {
		...
		while ($line) {
			list ($element, $value) = explode(": ", $line, 2);
                        workerLog('currentsong : element : ' . $element . ' value : ' . $value);
			...
			// All other tags
				...
				$array[$element] = $value;
                        ...
                        workerLog('currentsong : element : ' . $element . ' value : ' . $array[$element]);
...
function enhanceMetadata($current, $sock, $caller = '') {
        ...
        $current['title'] = isset($song['Title']) ? $song['Title'] : 'Unknown Title';
        $current['coverurl'] = isset($song['AlbumArtURI']) ? $song['AlbumArtURI'] : DEF_COVER;

        return $current;
}

function enhanceMetadata1($current, $sock, $caller = '') {
	$song = parseCurrentSong($sock);
	$current['file'] = $song['file'];
	...
	if ($current['file'] == null) {
		...
		// Song file, UPnP URL or Podcast
			...
			if (substr($song['file'], 0, 4) == 'http') {
                ...
                $current['coverurl'] = $song('AlbumArtURI');
